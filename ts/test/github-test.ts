...
testContextual.serial('should retry when unknown whether PR is mergeable', async (t) => {
...
testContextual.serial('should throw on max retries', async (t) => {
  ...
  const err: Error = await t.throwsAsync((t.context as WithRepo).repo.getPRCommits(12345, 11));
  scope.done();
  t.is(
      err.message,
...
testContextual.serial('should throw when PR is not mergeable', async (t) => {
  ...
  const err: Error = await t.throwsAsync((t.context as WithRepo).repo.getPRCommits(12345), Error);
  scope.done();
  t.is(err.message, 'PR is not mergeable');
});

testContextual.serial('should throw when PR merge commit sha is not found', async (t) => {
  const scope = nock('https://api.github.com')
                    .get('/repos/luke/star-destroyer/pulls/12345')
                    .reply(200, {
                      mergeable: true,
...
testContextual.serial('should throw when PR HEAD commit sha is not found', async (t) => {
  const scope = nock('https://api.github.com')
                    .get('/repos/luke/star-destroyer/pulls/12345')
                    .reply(200, {
                      mergeable: true,
...
testContextual.serial('retrieve file content', async (t) => {
  ...
  const content = await (t.context as WithRepo).repo.getFileContent('deadbeef', 'src/foo.js');
  scope.done();
  t.is(content, 'this is foo.js');
});

testContextual.serial('should retrieve package.json from a single repo', async (t) => {
  const scope =
      nock('https://api.github.com')
...
testContextual.serial(
    'should throw when package.json does not have contents', async (t) => {
      const scope = nock('https://api.github.com')
                        ...
                        .reply(200);
      const err: Error =
          await t.throwsAsync((t.context as WithRepo).repo.getPackageJsonFiles('deadbeef'), Error);
      scope.done();
      t.is(err.message, 'Content of package.json not found');
    });

testContextual.serial('should get all package.json files from monorepo', async (t) => {
  const topScope =
      nock('https://api.github.com')
          .get('/repos/luke/star-destroyer/contents/package.json?ref=deadbeef')
          .reply(200, {
